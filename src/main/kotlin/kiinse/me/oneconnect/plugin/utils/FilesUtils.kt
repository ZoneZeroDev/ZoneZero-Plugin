package kiinse.me.oneconnect.plugin.utils
...
class FilesUtils(private val plugin: OneConnect) {
    ...
    private fun checkTomlFileVersion(configFile: File) {
        ...
        if (newVersion > cfgVersion || newVersion < cfgVersion) {
            try {
                ...
                copyFile(fileName)
                OneConnect.sendLog(Level.WARNING, "Version mismatch found for file '&c${fileName}&6'. This file has been renamed to '&c${oldCfg.name}&6' and a new file '&c${fileName}&6' has been created")
            } catch (e: Exception) {
                OneConnect.sendLog(Level.WARNING, "An error occurred while copying the new version of the file '&c${fileName}&6'! Message:", e)
            }
        }
        deleteFile(tmpCfg)
    }
    ...
    fun createFile(file: File) {
        if (file.createNewFile()) OneConnect.sendLog(Level.CONFIG, "File '&d" + file.name + "&6' created")
    }

    @Throws(SecurityException::class)
    fun createDirectory(file: File) {
        if (file.exists()) deleteFile(file)
        if (file.mkdirs()) OneConnect.sendLog(Level.CONFIG, "Directory '&d" + file.name + "&6' created")
    }
    ...
    private fun copyFileMethod(oldFile: String, destFile: File) {
        ...
        if (inputStream != null) {
            try {
                ...
                OneConnect.sendLog(Level.CONFIG, "File '&d" + destFile.name + "&6' created")
            } catch (e: IOException) {
                OneConnect.sendLog(Level.WARNING, "Error on copying file '&c" + destFile.name + "&6'! Message:", e)
            }
        } else {
            OneConnect.sendLog(Level.WARNING, "File '&c$oldFile&6' not found inside plugin jar. Creating a new file...")
            try {
                createFile(destFile)
            } catch (e: IOException) {
                OneConnect.sendLog(Level.WARNING, "Error on creating file '" + destFile.name + "'! Message:", e)
            }
        }
    }
    ...
    fun copyFile(destFile: File) {
        if (!destFile.exists() || listFilesInDirectory(destFile).isEmpty()) {
            ...
            try {
                ...
                OneConnect.sendLog(Level.WARNING, "Error on copying directory '&c$directoryName&6'! Message:", e)
                deleteFile(destFile)
            }
        }
    }
    ...
    fun copyFileInFolder(file1: File, file2: File) {
        if (!file2.exists()) {
            ...
            OneConnect.sendLog(Level.CONFIG, "File '&d" + file1.name + "&6' copied to file '&d" + file2.name + "&6'")
        }
    }

    private fun accessFile(file: String): InputStream? {
        var input = OneConnect::class.java.getResourceAsStream(file)
        if (input == null) input = OneConnect::class.java.classLoader.getResourceAsStream(file)
        return input
    }
    ...
    private fun getResourceUrls(locationPattern: String): List<String?> {
        return Arrays.stream(PathMatchingResourcePatternResolver(OneConnect::class.java.classLoader).getResources(locationPattern))
    ...
    fun deleteFile(file: File) {
        if (file.delete()) OneConnect.sendLog(Level.CONFIG, "File '&d" + file.name + "&6' deleted")
    }
}