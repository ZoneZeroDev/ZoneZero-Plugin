package kiinse.me.oneconnect.plugin.service

import io.sentry.Sentry
import kiinse.me.oneconnect.plugin.OneConnect
import kiinse.me.oneconnect.plugin.enums.Config
import kiinse.me.oneconnect.plugin.service.enums.KeyType
import kiinse.me.oneconnect.plugin.service.enums.ServerAddress
import kiinse.me.oneconnect.plugin.exceptions.APIException
import kiinse.me.oneconnect.plugin.exceptions.SecureException
...
class ApiConnection(private val oneConnect: OneConnect) : ApiService {
    ...
    init {
        val config = oneConnect.configuration
        ...
        try {
            ...
            OneConnect.sendLog(Level.CONFIG, e)
    ...
    override fun get(address: ServerAddress): ServerAnswer {
        try {
            OneConnect.sendLog(Level.CONFIG, "-----------------------") // TODO: Убрать при релизе
            OneConnect.sendLog(Level.CONFIG, "Address: ${address.string}") // TODO: Убрать при релизе
            val request = Request.Get("$serviceServer/${address.string}")
                .connectTimeout(50000)
                .socketTimeout(50000)
                .addHeader("Content-Type", "application/json")
                .addHeader("publicKey", publicKeyString)
                .addHeader("Authorization", "Bearer ${oneConnect.token}")
    ...
    private fun getServerAnswer(response: HttpResponse): ServerAnswer {
        ...
        OneConnect.sendLog(Level.CONFIG, "Status: ${answer.status}") // TODO: Убрать при релизе
        OneConnect.sendLog(Level.CONFIG, "Body: ${answer.data}") // TODO: Убрать при релизе
        OneConnect.sendLog(Level.CONFIG, "-----------------------") // TODO: Убрать при релизе
        if (responseCode == 401) {
            if (body.has("message") && body.has("jwt") && body.getString("message").lowercase() == "your jwt has been updated!") {
                oneConnect.filesUtils.updateTomlKey("config.toml", body.getString("jwt"))
                oneConnect.onReload()
            }
        }
        return answer
    ...
    override fun updateServerKey() {
        try {
            OneConnect.sendLog(Level.CONFIG, "-----------------------") // TODO: Убрать при релизе
            OneConnect.sendLog(Level.CONFIG, "Address: server") // TODO: Убрать при релизе
